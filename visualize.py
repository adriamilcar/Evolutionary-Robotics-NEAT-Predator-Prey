...
def plot_ExtraStats(statistics, ylog=False, view=False, filename='avg_fitness.svg'):
    """ Plots the population's average and best fitness. """
    if plt is None:
        warnings.warn("This display is not available due to a missing optional dependency (matplotlib)")
        return
    ...
    generation = range(len(statistics.most_fit_genomes))
    best_fitness = [c.ExtraVar for c in statistics.most_fit_genomes]
    avg_fitness = np.array(statistics.get_fitness_mean())
    stdev_fitness = np.array(statistics.get_fitness_stdev())
    ...
    plt.title("Population's average and R of best genome")
    plt.xlabel("Generations")
    plt.ylabel("R in bits")
    plt.grid()
    plt.legend(loc="best")
    if ylog:
        plt.gca().set_yscale('symlog')
